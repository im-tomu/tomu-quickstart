 ...
 * Copyright (C) 2018 Michael Steil <mist64@mac.com>
 *
 ...
 * \addtogroup Examples
 *
 * This implements a USB CDC-ACM device (aka Virtual Serial Port),
 * which is an interface to the Commodore 64 BASIC interpreter running
 * inside a 6502 emulator.
 *
 */
...
volatile unsigned int read_index = 0;
...
static uint8_t input_get()
{
    ...
    uint8_t val = input_buf[read_index];
    read_index = (read_index + 1) % sizeof(input_buf);
    return val;
}

static void input_put(uint8_t val)
{
    if (write_index == (read_index - 1) % sizeof(input_buf)) {
        // buffer full :(
        return;
    }
    input_buf[write_index] = val;
    write_index = (write_index + 1) % sizeof(input_buf);
}
...
#if 0 // DEBUG
static void usb_puthex8(uint8_t a)
{
    char s[3];
...
static void usb_puthex16(uint16_t a)
{
    char s[5];
    const char hex[] = "0123456789abcdef";
    s[0] = hex[(a >> 12) & 15];
...
#define RAM_TOP sizeof(ram)
    ...
    while (1) {
        ...
        if (pc == 0xff90) {
            ...
            // block until serial connect
            if (!g_usbd_is_connected) {
                while (!g_usbd_is_connected);
                // delay a bit after the connect, otherwise the
                // first few characters will be lost
                udelay_busy(100*1000);
            }
